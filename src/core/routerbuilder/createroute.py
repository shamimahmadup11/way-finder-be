...
class RouteBuilder:
        ...
        try:
            for path in os.walk(self.dir_path):
                if len(path[2]) != 0 and path[2][0].endswith('.py'):
                    url_path, file_names = path[0][7:], path[2]
                    url_path = '/'.join(['{' + part[1:] + '}' if part.startswith('_') else part for part in url_path.split('/')])
                    self.module_dict[url_path] = [self.__load_module(path, file) for file in file_names]
    ...
    def __load_module(self, path: tuple, file: str):
        ...
        """
        try:
            file_path = os.path.join(os.getcwd(), path[0], file)
    ...
    def _set_path(self):
        ...
        try:
            for mod_name, modules in self.module_dict.items():
                # to handle a file when placed in the source directory then mod_name comes as '', this will add it to the route
                mod_name = '/' if mod_name == '' else mod_name
                for func in modules:
